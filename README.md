...
Status: day 5 of 7. The data pipeline, causal attention, RoPE, the loss,
...
verified on an A100, not only on CPU. Short probes of both experiment configs
have run. The full training runs, the KV cache and the benchmarks have not.

The headline results table below reads NOT MEASURED and stays that way until the
full runs produce the numbers. Probe figures appear separately and are labelled
as probes.
...
is why the spec warns against assuming a speedup. At 20M tokens that is 2.7
minutes for the dense run against 5.5 minutes for the MoE, so the matched token
budget is affordable either way.
...
Measured over the first 30 updates of the MoE probe, the four experts drift
away from uniform:

| Step | Assignment fractions | max/min |
| --- | --- | --- |
| 0 | 0.277, 0.234, 0.235, 0.254 | 1.18 |
| 10 | 0.268, 0.299, 0.188, 0.245 | 1.59 |
| 20 | 0.312, 0.303, 0.227, 0.158 | 1.97 |
| 29 | 0.286, 0.310, 0.215, 0.189 | 1.64 |

Uniform would be 0.250 each. This is imbalance forming, not collapse, and 30
updates is short enough that warmup had not finished, so treat the trend as
suggestive. The auxiliary loss moved with it, 1.021 to 1.076.
...
almost no information about load balance. Read the per-layer assignment
fractions, aggregated over a batch of updates rather than one.

## Results

The headline comparison needs the full 20M-token runs, which have not happened.

| Model | Body params | Total params | Width | Processed tokens | Val NLL | Perplexity | Train tok/s | Peak mem |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Dense + RoPE | 22,423,296 | 41,721,984 | m=4096 | NOT MEASURED | NOT MEASURED | NOT MEASURED | 122,100 | 4,139 MiB |
| Top-2 MoE + RoPE | 41,306,880 | 60,605,568 | 4×m=2048 | NOT MEASURED | NOT MEASURED | NOT MEASURED | 60,800 | 4,642 MiB |

### Probe runs, not results

Short runs used to size the experiment. None is long enough to say anything
about model quality.
